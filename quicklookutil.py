...
def archive_TO_elev_start_end(archive):
    ...
    if(ra < 1e-16):
        ra, dec = get_ra_dec(archive.get_filename(), deg=True)
        print('WARNING: There is an issue when extracting RA with psrchive. Will use a psredit command.')
    #print(ra, dec)
    ...
    mjd_array = Time(np.linspace(MJD0, MJDlast, 20), format='mjd')
    ...
    elevmean = np.mean(c.transform_to(AltAz(obstime=mjd_array, location=Site)).alt.degree)

    return elev0.alt.degree, elevlast.alt.degree, elevmean
...
def load_some_chan_in_archive_data(path, WORKDIR='NULL', initmetadata=False,
                                   ...
                                   bscrunch=1, tscrunch=1, fscrunch=1, pscrunch=False, dm=0.0, rm=0.0, defaraday=True):
    ...
    for i in range(len(files)):
        ...
        if (np.max(freqs) < minfreq) or (np.min(freqs) > maxfreq):
            continue

        if(np.min(freqs) < minfreq):
            minchan = (np.argmax(freqs[np.where(freqs <= minfreq)]))
        else:
            minchan = 0

        if(np.max(freqs) > maxfreq) and (np.min(freqs) < maxfreq):
        ...
        if (maxchan < buffarchive.get_nchan() - 1):
            buffarchive.remove_chan(int(maxchan + 1), int(buffarchive.get_nchan() - 1))
        if (minchan > 0):
            buffarchive.remove_chan(int(0), int(minchan - 1))
        if (pscrunch):
        ...
        if (maxchan < archives.get_nchan() - 1):
        ...
        if (first):
            ...
            newarchive_freq = newarchive.get_Profile(0, 0, 0).get_centre_frequency()
...
def get_ra_dec(fits_name, deg=False):
    commande_radec = 'psredit -c coord ' + fits_name + ' | cut -d = -f2'
    # print(commande_radec)
    radec = check_output(commande_radec, shell=True).decode("utf-8")
    ra = radec[:11]
    dec = radec[12:]
    PSR = SkyCoord(ra + ' ' + dec, unit=(u.hourangle, u.deg))
    # print(PSR.to_string('hmsdms'))
    if (deg == True):
        return PSR.ra.degree, PSR.dec.degree
    else:
        return str(PSR.ra.to_string(u.hour, sep=':')), str(PSR.dec.to_string(u.degree, sep=':'))
...
nb antenna   number of antennae                    %s""" % (UTC_START,
    ...
    if(str(ra) == '00:00:00.000'):
        print('WARNING: There is an issue when extracting RA with psrchive. Will use a psredit command.')
        ra, dec = get_ra_dec(archive.get_filename(), deg=False)
    ...
    elevStart, elevEnd, elevmean = archive_TO_elev_start_end(archive)
    ...
    SNR_norm = SNRvlad * np.sqrt(3600. / obs_duration) * (np.sin(90. * np.pi / 180.) / np.sin(elevmean * np.pi / 180.))**2 / ((100 - float(RFI)) / 100.)
...
dm           Dispersion measure (pc/cm^3)          %.6f
new dm       refined Dispersion measure (pc/cm^3)  %.6f
...
def auto_find_off_window(data, rot_bins, nbins, adjust):
    ...
    amean = np.mean(data[int(re - le):int(nbins)])
    arms = np.std(data[int(re - le):int(nbins)])
...
def auto_snr_onpulse(raw):
    ...
    range_profsign = np.sum(range_prof) / np.sqrt(np.abs(range_weq))
...
def reduce_pdf(pdf_path, pdf_name, dpi=400):
    ...
    try:
        ...
        print('WARNING: Command \'gs\' not found, but can be installed with:')
