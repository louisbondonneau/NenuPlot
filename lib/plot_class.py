...
class PlotArchive(RM_fit_class):
    ...
    def PA_vs_time(self, AX, rightaxis=False):
        ...
        datetime_array = []
        for isub in range(len(arx.times)):
            datetime_array.append(arx.times[isub].to_datetime())

        print(np.shape(arx.times.to_datetime()))
        print(np.shape(profil_PPA[:, best_bin]))
        print(np.shape(profil_PPA_err[:, best_bin]))

        AX.errorbar(arx.times.to_datetime(), profil_PPA[:, best_bin], yerr=profil_PPA_err[:, best_bin], fmt='r+', label='PPA (best bin)')
        AX.legend(loc='upper right', fontsize=6)
        AX.set_xlabel('Time (minutes)')
        if (rightaxis):
            # AXchan = AX.twinx()
